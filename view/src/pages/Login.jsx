...
export default function Login({ title = 'Welcome to Digital Legacy', expectedRole = null }) {
    ...
    const [selectedRole, setSelectedRole] = useState('Memory Owner');
    ...
    const handleAuth = async (e) => {
        ...
        try {
            if (isLogin) {
                ...
                await axios.post('http://127.0.0.1:5000/api/auth/register', { name, email, password, role: selectedRole });
   ...
   return (
    <div className="auth-container">
        ...
        {expectedRole === 'Memory Owner' && (
            <a href="/legacy-contact" style={{ position: 'fixed', top: '20px', left: '24px', color: 'var(--primary)', fontSize: '0.875rem', fontWeight: 600, textDecoration: 'none', background: 'rgba(99,102,241,0.12)', border: '1px solid var(--primary)', padding: '6px 14px', borderRadius: '8px', zIndex: 999 }}>
                Login as Legacy Contact →
            </a>
        )}

        {expectedRole === 'Legacy Contact' && (
            <a href="/login" style={{ position: 'fixed', top: '20px', left: '24px', color: 'var(--primary)', fontSize: '0.875rem', fontWeight: 600, textDecoration: 'none', background: 'rgba(99,102,241,0.12)', border: '1px solid var(--primary)', padding: '6px 14px', borderRadius: '8px', zIndex: 999 }}>
                ← Login as Memory Owner
            </a>
        )}
                    ...
                    <div>
                                ...
                                <>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                                            {['Memory Owner', 'Legacy Contact'].map(role => (
                                                <button
                                                    key={role}
                                                    type="button"
                                                    ...
                                                    style={{
                                                        ...
                                                        padding: '10px',
                                                        borderRadius: '8px',
                                                        border: `2px solid ${selectedRole === role ? 'var(--primary)' : 'var(--border-color)'}`,
                        ...
                        <p style={{ textAlign: 'center', marginTop: '2rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                            ...
                            <span
                                ...
                                onClick={() => { setIsLogin(!isLogin); setSelectedRole('Memory Owner'); }}
                            >
