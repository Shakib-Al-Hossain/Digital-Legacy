...
import { User, Activity, LogOut, Settings, Save, UserPlus, AlertTriangle, Shield, Moon, Sun, Archive, Mail } from 'lucide-react';
...
export default function Profile() {
    ...
    const handleDeactivate = () => {
        setWarningMsg("Are you sure you want to deactivate your account? Data export will be prepared.");
    };
            ...
            {warningMsg && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
                    <div className="glass-panel" style={{ width: '400px', textAlign: 'center' }}>
                        ...
                        <p className="mt-4 mb-4 text-muted">{warningMsg}</p>
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
                            <button className="btn btn-danger" style={{ width: '100px' }} onClick={confirmDeactivate}>Confirm</button>
                            <button className="btn" style={{ width: '100px', background: 'var(--bg-gradient-1)' }} onClick={() => setWarningMsg('')}>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
                    <div className="glass-panel" style={{ width: '400px', textAlign: 'center' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <button className="btn" style={{ width: '100px', background: 'var(--bg-gradient-1)' }} onClick={() => { setSuccessMsg(''); localStorage.removeItem('token'); navigate('/login'); }}>Close</button>
                        </div>
                    </div>
                </div>
            )}
