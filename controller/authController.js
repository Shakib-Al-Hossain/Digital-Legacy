...
exports.register = async (req, res) => {
    try {
        ...
        const allowedRoles = ['Memory Owner', 'Legacy Contact'];
        const assignedRole = allowedRoles.includes(role) ? role : 'Memory Owner';
        user = new User({ email, name, password, role: assignedRole });
...
exports.login = async (req, res) => {
    try {
        ...
        if (expectedRole && user.role !== expectedRole) {
            // If the actual user is an Admin, allow them through regardless of which login page they used
            if (user.role === 'Administrator') {
                // Do nothing — fall through to the 2FA flow below, admin will be redirected correctly after 2FA
            } else {
